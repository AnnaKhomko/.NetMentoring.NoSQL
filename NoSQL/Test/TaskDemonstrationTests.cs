...
{
	[TestClass]
	public class TaskDemonstrationTests
	{
		...
		{
			bookCollection = ConnectAndCreateMongoCollection();
		}
		...
		Console.WriteLine($"Book with min count (count - {minBook.Count}): {minBook.Name};\nBook with max count (count {maxBook.Count}): {maxBook.Name}.");
			...
			collection.Find(book => !String.IsNullOrWhiteSpace(book.Author)).ToList().Select(book => book.Author)
		...
		Console.WriteLine($"\nTask 5.\n");
			...
			collection.Find(book => String.IsNullOrWhiteSpace(book.Author)).ToList()
		...
		Console.WriteLine($"\nTask 6.\n");
			...
			collection.Find(book => true).ToList().ForEach(x => Console.WriteLine($"Count before increment :{x.Count}"));

			collection.UpdateMany(Builders<Book>.Filter.Empty,
				Builders<Book>.Update.Inc("Count", 1));

			collection.Find(book => true).ToList().ForEach(x => Console.WriteLine($"Count after increment :{x.Count}"));
		...
		private IMongoCollection<Book> ConnectAndCreateMongoCollection()
		{
		...
		private List<Book> FillInBookData()
		{
